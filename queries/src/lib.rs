...
pub async fn password_is_valid(
    user_password: ActiveValue<String>,
    actual_password_in_db: String,
) -> bool {
    let valid = verify(&user_password.as_ref(), &actual_password_in_db).unwrap();
    dbg!(valid)
}
